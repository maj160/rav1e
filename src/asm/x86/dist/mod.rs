...
#[target_feature(enable = "avx2")]
#[inline]
unsafe fn rav1e_sad_KxN_hbd_avx2_inner<const K : usize>(src: *const u8, src_stride: isize, dst: *const u8, dst_stride: isize, n_rows : usize) -> __m256i {
  ...
  let sum16 = (0..n_rows as isize).flat_map(|n| {
    (0..(K/16) as isize).map(move |x| unsafe {
      let a = _mm256_abs_epi16(_mm256_sub_epi16(
        _mm256_loadu_si256(src.offset(n * src_stride + x * LOADS_PER_REGISTER * 2) as *const _),
        _mm256_loadu_si256(dst.offset(n * dst_stride + x * LOADS_PER_REGISTER * 2) as *const _)
      ));
      a
    }) // We use reduce here as it lets the optimiser parallelise the summation.
  }).reduce(|a,b| unsafe { _mm256_add_epi16(a,b) }).unwrap();
