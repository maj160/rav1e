...
pub(crate) mod rust {
  ...
  pub fn get_sad<T: Pixel>(
    plane_org: &PlaneRegion<'_, T>, plane_ref: &PlaneRegion<'_, T>, _bit_depth: usize, _cpu: CpuFeatureLevel,
  ) -> u32 {
    debug_assert!(plane_org.rect().width <= 128 && plane_org.rect().height <= 128);
    ...
    plane_org
      ...
      .map(|(src, dst)| {
        src
          ...
          .zip(dst)
          .map(|(&p1, &p2)| {
            i32::cast_from(p1).abs_diff(i32::cast_from(p2))
          })
          .sum::<u32>() as u32
      })
      .sum()
    ...
    for chunk_y in (0..h).step_by(size) {
      ...
      for chunk_x in (0..w).step_by(size) {
        ...
        if chunk_w != size || chunk_h != size {
          sum += get_sad(
            &chunk_org, &chunk_ref, _bit_depth, _cpu,
          ) as u64;
          continue;
        }
            ...
            let sum = chunk1.rows_iter()
              ...
              .map(|(chunk_row1, chunk_row2)| {
                chunk_row1
                  ...
                  .zip(chunk_row2)
                  .map(|(&a, &b)| {
                    let c = (i32::cast_from(a) - i32::cast_from(b)) as i32;
                    (c * c) as u32
                  }).sum::<u32>()
                }).sum::<u32>();
            (sum as u64 * scale as u64 + (1 << GET_WEIGHTED_SSE_SHIFT >> 1)) >> GET_WEIGHTED_SSE_SHIFT
          }).sum::<u64>()
      }).sum::<u64>();
