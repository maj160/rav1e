    ...
    fi.rec_buffer.frames[fi.ref_frames[ref_frame.to_index()] as usize]
  {
    ...
    let org_region = &match ssdec {
      0 => ts.input_tile.planes[0]
        .subregion(Area::BlockRect { bo: tile_bo.0, width: w, height: h }),
      1 => ts.input_hres.region(Area::Rect { x: po.x, y: po.y, width: w, height : h }),
      2 => ts.input_qres.region(Area::Rect { x: po.x, y: po.y, width: w, height : h }),
      _ => unimplemented!(),
    };
...
fn full_pixel_me<T: Pixel>(
  ...
  po: PlaneOffset, lambda: u32, pmv: [MotionVector; 2],
...
fn sub_pixel_me<T: Pixel>(
  ...
  mvx_max: isize, mvy_min: isize, mvy_max: isize, 
...
fn subpel_diamond_search<T: Pixel>(
  ...
  mvx_min: isize, mvx_max: isize, mvy_min: isize, mvy_max: isize, use_satd: bool, current: &mut MotionSearchResult,
  ref_frame: RefType,
) {
...
fn get_fullpel_mv_rd<T: Pixel>(
  ...
  compute_mv_rd(
    fi, pmv, lambda, use_satd, bit_depth, cand_mv, org_region,
    &plane_ref,
  )
}
...
fn get_subpel_mv_rd<T: Pixel>(
  ...
  mvx_min: isize, mvx_max: isize, mvy_min: isize, mvy_max: isize, cand_mv: MotionVector, tmp_region: &mut PlaneRegionMut<T>,
  ref_frame: RefType,
) -> MVCandidateRD {
  if (cand_mv.col as isize) < mvx_min
    || (cand_mv.col as isize) > mvx_max
...
fn full_search<T: Pixel>(
  ...
  org_region: &PlaneRegion<T>, p_ref: &Plane<T>,
